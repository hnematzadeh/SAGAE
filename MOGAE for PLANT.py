# -*- coding: utf-8 -*-
"""
Created on Wed May  6 20:23:57 2026
...
######################################################
#  5, 13 = Black Spot,   29 = Canker,  46,38,35 = Greening,  55 = Healthy
X = X1_test[42].astype(np.float32)
Y = y1_test[42]
...
superpixels= slic(image, n_segments=5, compactness=10, sigma=1, start_label=0, channel_axis=-1) 
nVar = len(np.unique(superpixels))
el=0
TEMP=0
arTEMP=[]
limit=6
...
image=X1_test[Figure]
class_to_explain=np.where(y1_test[Figure]==1)
superpixels= slic(image, n_segments=50, compactness=10, sigma=1, start_label=0, channel_axis=-1) 
...
# 1. Start with your leaf image (the perturbed/masked version)
# We need to make sure it is (256, 256, 3)
if black_back.ndim == 2:
    # If it's grayscale, stack it to create 3 identical RGB channels
    prediction_input = np.stack((black_back,)*3, axis=-1)
elif black_back.ndim == 3 and black_back.shape[-1] == 1:
    # If it's (256, 256, 1), repeat the single channel 3 times
    prediction_input = np.concatenate([black_back] * 3, axis=-1)
else:
    # Already (256, 256, 3)
    prediction_input = black_back

# 2. Add the Batch dimension to make it (1, 256, 256, 3)
prediction_input = np.expand_dims(prediction_input, axis=0)

# 3. Double check the shape! 
# It MUST be (1, 256, 256, 3) for the Citrus Leaf model.
print(f"Feeding shape to model: {prediction_input.shape}") 

# 4. Predict
prediction = model.predict(prediction_input)

# We use class_to_explain to see how the score changes for that specific disease
score = np.round(prediction[0][class_to_explain], 2)

print(f"Success! Score for Citrus class {class_to_explain}: {score}")
################################################################
######################## sparsity #############################
import numpy as np
...
def FS(individual, model, target_class, mask):
    ...
    # 1. Prepare the input for the Citrus Model
    # individual is (256, 256, 3). We just need to add the batch dimension.
    # No more np.mean(axis=2), as we need the color information for disease detection.
    temp_final = individual[np.newaxis, ...].astype('float32')

    # 2. Ensure normalization
    # If the individual is in 0-255 range, scale to 0-1
    ...
    # 4. Correct sparsity (mask-based)
    # This remains the same: percentage of the image that is "off"
    sparsity = 1 - np.mean(mask)

    return fidelity, sparsity

# --- Execution ---
# Note: target_class is 3 (ensure this matches your citrus label index)
binary_mask = np.any(black_back != 0, axis=-1).astype(int)
f, s = FS(black_back, model, 2, binary_mask)
...
mask_to_intersect = force_rgb(final_mask_3d)
...
# Example Usage:
stability_score = evaluate_stability(image, model, class_to_explain, n_runs=3)
